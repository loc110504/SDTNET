...
parser.add_argument('--exp', type=str,
                    default='XNetv2', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled in enumerate(trainloader):
            ...
            # TAS loss
            loss_ce_i  = ce_loss(logits, scrib)
            loss_ce_LF = ce_loss(logits_LF, scrib)
            loss_ce_HF = ce_loss(logits_HF, scrib)
            loss_TAS = loss_ce_i + loss_ce_LF + loss_ce_HF
            if loss_ce_LF.item() <= loss_ce_HF.item():
                y_win = y_LF
                logits_win = logits_LF
                loss_ce_win = loss_ce_LF
            else:
                y_win = y_HF
                logits_win = logits_HF
                loss_ce_win = loss_ce_HF
            # BAP
            with torch.no_grad():
                denom = (loss_ce_i.detach() + loss_ce_win.detach()).clamp_min(1e-8)
                w_i = (loss_ce_win.detach() / denom).item()
                w_win = (loss_ce_i.detach() / denom).item()
            mixed_prob = w_i * y + w_win * y_win
            y_pl = torch.argmax(mixed_prob.detach(), dim=1)
            loss_PL = dice_loss(y_LF, y_pl.unsqueeze(1)) + dice_loss(y_HF, y_pl.unsqueeze(1)) + dice_loss(y, y_pl.unsqueeze(1))
            ...
            B_i = exrct_boundary(y,    iter_=1)
            loss_BD = bd_loss_fn(B_LF, B_pl.detach()) + bd_loss_fn(B_HF, B_pl.detach()) + bd_loss_fn(B_i, B_pl.detach())
