...
parser.add_argument('--exp', type=str,
                    default='TABNet_new_PL_noCut', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled in enumerate(trainloader):
            ...
            y_pl = alpha * y_j.detach() + (1 - alpha) * y_k.detach()
            loss_PL = 0.5 * (ce_loss2(y_j, y_pl) + ce_loss2(y_k, y_pl))
            y_pl = torch.argmax(y_pl.detach(), dim=1)  
